/***********************************************
Ninebot_Sign_Single_v2.9.1【盲盒字段修复终版】
// 修复点：waitDay替换leftDaysToOpen、receive领盒、open传boxId、自动开箱失效根治
适配：Surge/Quantumult X/Loon/Scripting
功能：自动签到/补签/每日领盲盒+全盲盒自动开箱/车辆数据抓取存BoxJs
***********************************************/
...
async function captureVehicleData() {
    ...
    try {
        ...
        if (!d) return;
        const write = (k, v) => v !== undefined && writePS(String(v), k);
...
async function openAllAvailableBoxes(headers) {
    ...
    try {
        ...
        const allNotOpened = boxResp?.data?.notOpenedBoxes || [];
        // 修复：使用waitDay=0筛选可开盲盒
        const availableBoxes = allNotOpened.filter(b => Number(b.waitDay ?? 0) === 0);
        ...
        for (const box of availableBoxes) {
            ...
            try {
                const openResp = await httpPost(END.blindBoxOpen, headers, { boxId: boxId }, "blindBox");
                if (openResp.code === 0) {
        ...
        if (!isTodaySigned) {
            ...
            try {
                ...
                if (signResp?.code === 0 && Array.isArray(signResp.data?.rewardList)) {
                    ...
                    try {
                        const getBoxRes = await httpPost(END.blindBoxReceive, headers, {}, "blindBox");
                        if (getBoxRes.code === 0) {
                            logInfo("✅当日盲盒领取成功");
                        } else {
        ...
        if (cfg.notify) {
            ...
            try {
                ...
                blindProgress = notOpened.length > 0
                    ? `- 待开盲盒：${notOpened.length}个（可开：${notOpened.filter(b => Number(b.waitDay ?? 0) === 0).length}个）`
                    : "- 待开盲盒：0个";
